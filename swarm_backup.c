...
int main(int argc, char *argv[])
{
  ...
  while (true)
  {
    ...
    if (simulation_started)
    {
      ...
      int destination_bearing = get_destination_bearing(x, y, xD, yD);

      //printf("%f \n",destination_bearing);

      /*Get Compass Values*/
      const double *north = wb_compass_get_values(compass);
      int robot_bearing_degree = get_robot_bearing(north[0], north[2]);
      ...
      for (i = 0; i < 8; i++)
      {
        ...
        else
        {
          ...
          if (abs(destination_bearing - robot_bearing_degree) < 3)
          {
            if (distance < 0.1)
            {
